        ...
        template: {
          ...
          spec: {
            containers: [{
              ...
              command: ['/bin/sh', '-c'],
              args: [`curl -sLO "https://dl.k8s.io/release/v1.31.0/bin/linux/amd64/kubectl" && chmod +x kubectl && mv kubectl /usr/local/bin/ && echo "Health monitor started" && while true; do CRASHLOOP=$(kubectl get pods -n motoros-prod -o json 2>/dev/null | grep -c CrashLoopBackOff || echo 0); OOM=$(kubectl get pods -n motoros-prod -o json 2>/dev/null | grep -c OOMKilled || echo 0); DTC_COUNT=$(kubectl exec deploy/fleet-registry-svc -n motoros-prod -- env DB_HOST=$DB_HOST DB_USER=$DB_USER DB_PASS=$DB_PASSWORD python3 -c "import psycopg2,os;conn=psycopg2.connect(host=os.environ['DB_HOST'],port=5432,dbname='motoros',user=os.environ['DB_USER'],password=os.environ['DB_PASS'],connect_timeout=5);cur=conn.cursor();cur.execute(\\"SELECT COUNT(*) FROM dtc_events WHERE status='ACTIVE' AND last_seen_at > NOW()-interval '1 hour'\\");print(cur.fetchone()[0]);conn.close()" 2>/dev/null || echo 0); aws cloudwatch put-metric-data --namespace "MotorOS/Kubernetes" --metric-data "[{\\"MetricName\\":\\"CrashLoopBackOffPods\\",\\"Value\\":$\{CRASHLOOP},\\"Unit\\":\\"Count\\"},{\\"MetricName\\":\\"OOMKilledPods\\",\\"Value\\":$\{OOM},\\"Unit\\":\\"Count\\"}]" --region ${cdk.Stack.of(this).region} 2>/dev/null; aws cloudwatch put-metric-data --namespace "MotorOS/VehicleHealth" --metric-data "[{\\"MetricName\\":\\"ActiveDTCCount\\",\\"Value\\":$\{DTC_COUNT},\\"Unit\\":\\"Count\\"}]" --region ${cdk.Stack.of(this).region} 2>/dev/null; echo "$(date -u +%H:%M:%S) crashloop=$CRASHLOOP oom=$OOM dtc=$DTC_COUNT"; sleep 30; done`],
              resources: { requests: { cpu: '50m', memory: '128Mi' }, limits: { cpu: '200m', memory: '256Mi' } },
            }],
          },
        },
